...
const pattern = date.compile('ddd, MMM D YYYY');
  data["data"].map((item) =>
    ...
    if (item.caption != undefined){
      w = <View style={styles.box} key={item.mediu_url}><Image source={{uri: item.media_url}} style={styles.image} key={item.mediu_url}/><Text key={item.caption}>{datee}{"\n"}{item.caption}</Text></View>
      raw_list.push(w);
    }else{
      w = <View style={styles.box} key={item.mediu_url}><Image source={{uri: item.media_url}} style={styles.image} key={item.mediu_url}/><Text key={item.caption}>{datee}</Text></View>
      raw_list.push(w);
    }
...
console.log(date.format(new Date(2020, 4, 5), pattern)); // => May 5 2020
  ...
  render(){
    ...
    return (
      ...
      <View>{
        listItems.map((item) =>
        <View style={styles.box} key={item.mediu_url}><Image source={{uri: item.media_url}} style={styles.image}/></View>
      )}
      </View>
